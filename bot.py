...
- Do not assume or project any human gender (do not say "kha raha hoon" or "kha rahi hoon" in a gender-biased way). Be purely neutral and conversational.
- Never use garbage or glitchy special symbols (*!:£/() etc.) unnecessarily. Keep sentences clean and meaningful.
...
def needs_thinking_process(text):
    \"\"\"Determine if a query is complex enough to show a Thinking Process box.\"\"\"
    t = (text or "").lower()
    
    # Casual or short queries should NOT show thinking process
    ...
    # Complex keywords indicating hard math, science, coding, analysis, or deep reasoning
    complex_triggers = [
...
async def generate_image_answer(
    ...
    user_prompt = prompt.strip() if prompt.strip() else "Analyze this image carefully."
    ...
    for model in MODEL_CANDIDATES:
        try:
            ...
            last_error = exc
            logger.exception("Image analysis failed model=%s", model)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = (
        "<b>Kiva AI</b>\n\n"
        f"Namaste / Hello {name}!\n\n"
        "Main Kiva AI hoon. Jo bhi poochna ho seedha message karo, mast baat karenge!"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = (
        "<b>Kiva AI Help</b>\n\n"
        "Apna sawal seedha bhejo. Hard questions aur coding ke liye thinking process aayegi, baaki normal chats direct rahengi."
    )
    await update.message.reply_text(message, parse_mode="HTML")
...
async def owner_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("Copy Owner Details", copy_text=CopyTextButton(text=owner_copy)),
            InlineKeyboardButton("Contact", url=OWNER_URL),
        ]
    ])
    message = (
        "<b>Kiva AI Owner</b>\n\n"
        f"<b>Founder & Developer:</b> {html.escape(OWNER_NAME)}\n"
...
async def send_answer(update, answer, citations=None):
    ...
    if citations:
        ...
        for title, url in citations[:6]:
            ...
            source_lines.append(f'• <a href="{safe_url}">{safe_title}</a>')
        formatted = formatted + "\n" + "\n".join(source_lines)
...
async def text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        ...
        typing_task = asyncio.create_task(
            typing_loop(context.bot, update.effective_chat.id, stop_event)
        )

        thinking_message = None
        try:
            # Check if this question warrants a thinking process display
            is_complex = needs_thinking_process(prompt)
            if is_complex:
                thinking_message = await send_thinking_message(update)

            answer, citations, _ = await generate_text(
                user.id,
                prompt,
            ...
            if "429" in error_msg or "quota" in error_msg.lower():
                message = "Arre yaar, abhi API limit / quota exceed ho gaya hai. Ek minute baad try karo, tab tak sab theek ho jayega!"
            elif needs_web_search(prompt):
                message = "Is question ke liye web search zaroori thi par abhi connection error hai. Dobara try karo!"
            else:
                message = "Kuch technical problem aa gayi hai, thodi der me dobara message karo."
        ...
        try:
            ...
            if update.message.photo:
                ...
                if not doc_mime.startswith("image/"):
                    await update.message.reply_text("Please valid image bhej bhai.")
                    return
            ...
            answer, citations = await generate_image_answer(
                user.id, prompt, get_display_name(user), image_bytes, mime_type
            )
            await send_answer(update, answer, citations)

        except Exception:
            logger.exception("Image processing failed")
            await update.message.reply_text("Image analyze karne me issue aaya, dobara bhej.")
